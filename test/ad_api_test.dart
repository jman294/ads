...
void main() {
  ...
  group('AdApi', () {
    test('.image() should have correct response for good id', () async {
      ...
      expect((await adApi.image(new Id(1))).statusCode, equals(200));
    });
    ...
    test('.text() should have correct response for good id', () async {
      ...
      expect((await adApi.text(new Id(1))).statusCode, equals(200));
    });
    ...
    test('.text() should contain tagline on good id', () async {
      expect((await adApi.text(new Id(1))).data, equals(new Utf8Encoder().convert(cPool.mockResults.list[0])));
    });

    test('.click() should have correct response for good id', () async {
      expect((await adApi.click(new Id(1))).headers['location'], equals(cPool.mockResults.list[0]));
    ...
    test('.click() should contain no body on good id', () async {
      cPool.mockResults.list = [''];
      adApi = new AdApi(cPool);
      expect((await adApi.click(new Id(1))).data, equals(new Utf8Encoder().convert(cPool.mockResults.list[0])));
    });
  });
}